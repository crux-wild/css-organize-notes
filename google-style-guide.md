...
使用字母顺序作为属性书写顺序，字母顺序更容易记忆和维护。
如果是含有厂商前缀的属性(例: `-moz-border-radius`)的顺序应根据与之对应标准属性(
例: `border-radius`)的位置。多个厂商前缀之间的顺序应该遵循字母顺序(例：`-moz`前
缀排在`-webkit`前缀的前面)。
...
在属性名的后面保持一个空格(而不是没有空格)，是基于代码一致性的考虑。
