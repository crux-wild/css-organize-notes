...
`HTML`和`CSS`的符号统一使用小写：包括`HTML`的元素和属性以及属性数值，`CSS`的选择
符和属性性值。
...
推荐使用`css`提供的类似`font`属性的简写方式，即使只需要设置单一属性值。
...
> `css3`提供了`rgba()`，`hsl()`，`hsla()`三种全新的语法。十进制(`dec`)较之十六进
> (`hex`)更加易于读写，增加`alpha`(透明度)通道也使语法上更具表现力。
>
...
使用`-`(中划线)作为连字符，可以提高选择符更加易读性和可拓展性。
...
如果所定义样式在不同浏览器中呈现不一致时。基于可维护性的考虑，`CSS hacks`应该作
为优先级最低的解决方案。
...
属性排列顺序尊许字母顺序，字母顺序更容易记忆与维护。
含有厂商前缀的属性(例: `-moz-border-radius`)的顺序应根据与之对应的标准属性(例: `
border-radius`)的位置。多个含有厂商前缀的属性彼此的排列顺序应该遵循字母顺序(例：
`-moz`前缀排在`-webkit`前缀的前面)。
...
所有代码块和属性集都使用缩进，可以增强代码的层次结构和提高可读性。
...
在属性名的冒号后面保持一个空格(而不是没有空格)，是基于代码一致性的考虑。
