...
> (`hex`)更加易于读写，增加透明度(`alpha`)通道也使语法上更具表现力。
>
...
如果所定义样式在不同浏览器中呈现不一致的时。基于可维护的考虑，`CSS hacks`应该作
为优先级最低的解决方案。
...
含有厂商前缀的属性(例: `-moz-border-radius`)的顺序应根据标准属性(例: `border-rad
ius`)的位置。多个含有厂商前缀的属性之间的顺序应该遵循字母顺序(例：`-moz`前缀排在
`-webkit`前缀前面)。
...
所有代码块和属性集都采用缩进，可以体现代码的层次结构和提高代码可读性。
...
属性选择器和属性值使用`'`(单引号)代替`"`(双引号)。URL属性(`url()`)中的引号省略不
写。
...
通过块注释划分一组功能相似的代码放到相同的地方。不同代码块之间使用`\n`(空行)作为
分隔。
